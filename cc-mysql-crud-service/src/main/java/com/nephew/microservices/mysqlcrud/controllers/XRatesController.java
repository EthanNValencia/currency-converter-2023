package com.nephew.microservices.mysqlcrud.controllers;

import java.util.ArrayList;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.nephew.common.entities.XRatesUsdData;
import com.nephew.microservices.mysqlcrud.services.XRatesUsdDataService;

@RestController
@RequestMapping("/cc-mysql-crud/x-rates/")
public class XRatesController {
	
	@Autowired
	private XRatesUsdDataService service;

	@PostMapping("/save-all")
	public void saveAllXRates(@RequestBody ArrayList<XRatesUsdData> list) {
		service.saveAll(list);
	}
	
	@GetMapping("/get-all")
	public List<XRatesUsdData> getAllXRates() {
		return service.getAll();
	}
	
}
